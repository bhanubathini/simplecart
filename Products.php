...
class Products{
        ...
        if($pnum > 0)
        {
          ...
          while($productRow = mysqli_fetch_array($productsQuery))
          {
            ...
            $data[] = array('productID' => $productRow['productID'],
              ...
              'image' => $this->imageurl($productRow['Image']),
  ...
  function getOrders($params){
    ...
    $sql=$this->db->query("select o.*, u.*, ab.*, o.CreatedOn OrderDate from {$statement} 
      INNER JOIN users u ON (o.UserId=u.UserId)
      INNER JOIN addressbook ab ON (ab.UserId=u.UserId AND ab.AddressId=o.AddressId)
      WHERE o.UserId = '".$params['userid']."' order by o.CreatedOn desc LIMIT {$startpoint} , {$limit}");
      ...
      if ($ordersCount > 0) {
        while($row = mysqli_fetch_array($sql)){
           ...
           $order['name'] = $row["Firstname"]." ".$row['Lastname'];
           ...
           $pq = $this->db->query("SELECT p.*, op.`Price`, op.`Quantity` FROM orderedproducts op
           INNER JOIN products p ON (p.productID=op.ProductId) WHERE orderId=".$row['orderId']) or die(mysqli_error($api->db));
           //echo mysqli_num_rows($pq);
           while($rp = mysqli_fetch_array($pq))
           {
            $products['ProductId'] = $rp['ProductId'];
            $products['ProductName'] = $rp['ProductName'];
            $products['Price'] = $rp['Price'];
            $products['Quantity'] = $rp['Quantity'];
            $order[$row["orderId"]]['products'][] = $products;
           ...
           $data['orderdetails'][$row["orderId"]][] = $order;
          }
         $data['orderdetails'] = array_values($data['orderdetails']);
         $data['Status'] = 1;
       
      } else {
        $data['Status'] = 0;
        $data['orders'] = "We have no orders";
      } 
      echo json_encode($data);
  }
